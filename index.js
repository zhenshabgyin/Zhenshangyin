...
class ZhenshangyinLinkage {
    constructor(options) {
        this.inputElements = options.multiInputs ? options.multiInputs.map(selector => document.querySelector(selector)) : [document.querySelector(options.container)];
    ...
    bindEvents(inputElement, level) {
        inputElement.addEventListener("click", (event) => {
            ...
            this.populateList(currentData, list);
    ...
    updateSelection(level, value) {
        if (this.isMultiInputMode) {
            ...
            if (level < this.inputElements.length - 1) {
                this.clearLowerLevelMenus(level + 1);
            ...
            const currentValue = inputElement.value.split(this.separator).slice(0, level);
            if (level > 0) {
                currentValue[level] = value;
            } else {
                currentValue[level] = value;
            }
            inputElement.value = currentValue.join(this.separator);
            if (level < this.inputElements.length - 1) {
                this.clearLowerLevelMenus(level + 1);
            }
        }
    ...
    getSubData(level) {
        ...
        for (let i = 0; i < level; i++) {
            const value = this.inputElements[i].value;
            const selected = data.find(item => item.name === value);
            data = selected ? selected.children : [];
        }
        return data;
    }
    getFullSelection() {
        ...
        return this.inputElements[0].value;
    }
